function varargout = view_examples(data_set, varargin)
% view_examples
%
% optional inputs:
%  (1) block
...
%% plot an example of two blocks (no-cues, high & low; np & p  / hpt & lpt)
...
figure; hold on;
x_temp =  b_data.x; y_temp = b_data.y;
plot(x_temp(:, hpt_tr & nocue_tr), y_temp(:, hpt_tr & nocue_tr), 'k', 'LineWidth', 2);
x_temp =  b_data.x; y_temp = b_data.y;
plot(x_temp(:, lpt_tr & nocue_tr), y_temp(:, lpt_tr & nocue_tr), 'r');
...
figure; hold on;
x_temp =  b_data.x; y_temp = b_data.y;
plot(x_temp(:, lpt_tr & pred_tr), y_temp(:, lpt_tr & pred_tr), 'r');
x_temp =  b_data.x; y_temp = b_data.y;
plot(x_temp(:, hpt_tr & pred_tr), y_temp(:, hpt_tr & pred_tr), 'k', 'LineWidth', 2);
title('Pred')


figure; hold on;
x_temp =  b_data.x; y_temp = b_data.y;
plot(x_temp(:, lpt_tr & unpred_tr), y_temp(:, lpt_tr & unpred_tr), 'r');
x_temp =  b_data.x; y_temp = b_data.y;
title('Un-Pred')
plot(x_temp(:, hpt_tr & unpred_tr), y_temp(:, hpt_tr & unpred_tr), 'k', 'LineWidth', 2);
...
a_temp = b_data.ma;
t_temp = b_data.t;
plot(t_temp(:, hpt_tr & unpred_tr), a_temp(:, hpt_tr & unpred_tr), 'k', 'LineWidth', 2);
plot(t_temp(:, lpt_tr & unpred_tr), a_temp(:, lpt_tr & unpred_tr), 'r');